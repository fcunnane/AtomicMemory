...
# ---- Auto-detect MASTER path ----
proc auto_detect_master {} {
    # Query all available 'master' services in System Console
    set paths [get_service_paths master]
    if {[llength $paths] == 0} {
        error "No 'master' services found. Is the board programmed and JTAG cable connected?"
    }

    # Prefer DE-SoC / 5CSE devices if present
    set preferred {}
    foreach p $paths {
        if {[regexp {DE-SoC|5CSEBA6|5CSEMA6} $p]} {
            lappend preferred $p
        }
    }
    ...
    if {[llength $paths] > 1} {
        puts "NOTE: multiple master paths found; using the first of:"
        foreach p $paths {
            puts "  $p"
        }
    }

    set chosen [lindex $paths 0]
    puts "Using MASTER path: $chosen"
    return $chosen
}

# ---- User config (derived) ----
set MASTER [auto_detect_master]
